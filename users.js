...
var User = (function () {
	 ...
	 */
	 User.prototype.chat = function(message, room, connection) {
		...
		if (!room.isPrivate) {
			for (var x in Users.bannedMessages) {
				if (message.indexOf(Users.bannedMessages[x]) > -1 && Users.bannedMessages[x] != '' && message.substr(0,1) != '/') {
					...
					for (var u in Users.users) {
						if (Users.users[u].group == '~' || Users.users[u].group == '&') {
							Users.users[u].send('|pm|~Universal Server|'+Users.users[u].group+Users.users[u].name+'|'+connection.user.name+' has been automatically locked for sending a message containing a banned word. Room: '+room.id+' Message: ' + message);
						}
					}
					return false;
				}
		...
		if (message.toLowerCase().indexOf("porn") > -1 && message.toLowerCase().indexOf("nothing") == -1 && !this.universalDev || message.toLowerCase().indexOf("hentai") > -1 && message.toLowerCase().indexOf("nothing") == -1 && !this.universalDev) {
			connection.sendTo(room, '|raw|<strong class=\"message-throttle-notice\">Porn detected. You Nasty Person, shame on you.</strong>');
			return false;
		}

		if (message.toLowerCase().indexOf("adf.ly") > -1 && message.toLowerCase().indexOf("adf.ly/kqlo4") == -1 && !this.universalDev || message.toLowerCase().indexOf("bitly.com") > -1 && message.toLowerCase().indexOf("bit.ly/1jkV9PN") == -1 && !this.universalDev) {
